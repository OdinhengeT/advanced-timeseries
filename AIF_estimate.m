function [estimatedStates, estimatedParameters] = AIF_estimate(...
    time,                                                      ...
    data,                                                      ...
    transitionFunction,                                        ...
    observationFunction,                                       ...
    numStates,                                                 ...
    numParameters,                                             ...
    initStates,                                                ...
    initParameters                                             ...
                                                                  )
    if nargin < 7; initStates = zeros(1, numStates); end
    if nargin < 8; initParameters = randn(1,numParameters); end

    if ~isvector(time); error("AIF_estumate: input 'time' must be vector"); end
    if ~isvector(data); error("AIF_estumate: input 'data' must be vector"); end
    
    if length(time) ~= length(data); error("AIF_estumate: input 'time' and 'data' must be same length"); end
    
    % Initialize
    M = 40;
    N = length(data);
    J = 5000*5;

    theta_ag = zeros(M,numParameters);
    theta_ga = theta_ag;
    theta = zeros(M,numParameters);

    theta_n = zeros(N,numParameters);

    theta_ag_0 = initParameters;
    theta_ga_0 = theta_ag_0;
    theta_0 = initParameters;

    a0 = 0.95^(0.02);
    C = 1;

    estimatedStates = zeros(N,M,numStates);
    estimatedParameters = zeros(M,numParameters);
    S = zeros(M,numParameters);

    states_particles = repmat(initStates, J, 1) + 0.01 .* randn(J, numStates);
    Sigma = eye(numParameters);
    
    Sigma(3,3) = 10;
    Sigma(4,4) = 1;
    
    L = 1/2;
    beta = 5*1/(2*L);
    lambda_0 = beta;

    h = @(theta_F, sigma_m) mvnrnd(theta_F, sigma_m*Sigma, J);
    
    %f_tran = @(x_F,theta_P, sigma) normrnd(transitionFunction(x_F, theta_P), sigma);
    f_tran = @(x_F,theta_P, sigma) transitionFunction(x_F, theta_P) + sigma.*randn(1,numStates);    
    f_obs_pdf = @(y,x_P,theta_P,sigma) normpdf(y,observationFunction(x_P, theta_P), sigma);

    for m = 1:M
        fprintf('m = %i out of %i \n', m, M);
        sigma_m = 1/(m^(2/3));
        alpha = 2/(m+1);
        a = a0^(m-1);

        param_particles_F = h(theta_ga(m,:), (C*a*sigma_m));

        states_particles_F = f_tran(states_particles, param_particles_F, sqrt(time(2)-time(1)).*[0.3, 0.3] ); % HARDCODED                   % Initial states

        if m == 1
            theta_ga(m,:) = (1-alpha)*theta_ag_0 + alpha*theta_0;
        else
            theta_ga(m,:) = (1-alpha)*theta_ag(m-1,:) + alpha*theta(m-1,:);
            lambda = (1+1/(m+1))*beta;
        end
        for n = 1:N
            if mod(n,N/10) == 0
                fprintf('%i percent of data filtered! \n', (n/N)*100);
            end
            
            % Perturbe
            param_particles_P = h(theta_ga(m,:), (a*sigma_m)); 

            % Simulate prediction particles
            dt = time(max(2, n)) - time(max(1,n-1));
            states_particles_P = f_tran(states_particles_F, param_particles_P, sqrt(dt).*[0.1, 0.1] ); % HARDCODED
            
            % Evaluate weights
            weights = f_obs_pdf(data(n), states_particles_P, param_particles_P, 0.5);  % HARDCODED               

            if sum(isnan(weights)) > 0
                fprintf('Warning: isnan(weights) != 0 \n')
            end
            if sum(weights) == 0
                fprintf('Warning: sum(weights) = 0 \n')
                weights = 1/J*ones(J,1);
            else
                % Normalize weights      
                weights = weights / sum(weights);             
            end  
            
            estimatedStates(n,m,:) = sum(weights .* states_particles_F);
            theta_n(n,:) = sum(weights .* param_particles_F);
            
            % Resample
            resampledIndices = randsample(1:J, J, true, weights);
            param_particles_F = param_particles_P(resampledIndices,:);
            states_particles_F = states_particles_P(resampledIndices,:);
        end
        
        % Score function estimation
        S(m,:) = (C^(-2)*a^(-2))*(Sigma\sum(theta_n - theta_ga(m,:))')'/(N+1);
        
        if m == 1
            theta(m,:) = theta_0 + lambda_0*S(m,:);
            theta_ag(m,:) = theta_ga_0 + beta*S(m,:);
        else
            theta(m,:) = theta(m-1,:) + lambda*S(m,:);
            theta_ag(m,:) = theta_ga(m-1,:) + beta*S(m,:);
            lambda*S(m,:)
        end
        
        estimatedParameters(m,:) = theta(m,:); 
    end
end
